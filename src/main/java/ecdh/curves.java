...
public class curves {
  ...
  public static String serializePointToString(ECPoint ecPoint) {
    if (ecPoint.equals(ECPoint.POINT_INFINITY)) {
      ...
      return MessageFormat.format("AFFINEX{0}AFFINEY{1}", ecPoint.getAffineX().toString(16),
          ecPoint.getAffineY().toString(16));
    }
  ...
  public ECPoint power(ECPoint point, BigInteger i) {
    // for info, this is power in the sense of group theory, in other context you
    // could call that a scalar multiplication
  ...
  public static ECPoint addTwoPoints(ECPoint a, ECPoint b) {
    ...
    if (a.getAffineX().mod(curves.p).equals(b.getAffineX().mod(curves.p))) {
      ...
      // if the xs are the same, but not the ys, it means we have the inverse of the
      // point (in the context of an additive group, we should call this opposite)
      else {
        return ECPoint.POINT_INFINITY;
      }
    }
